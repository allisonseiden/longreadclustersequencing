...
non_na_ids = [i for i in non_na_ids if i in age_df.ID.tolist()]
...
# NAs are from IDs that were not completely phased
...
def get_indel_df(indel_phased_df, indel_list, age_df):
    ...
    # remove the 0 parental age!
    rm_lines = indel_class_df.Maternal_age_at_conception == 0
    indel_class_df = indel_class_df[~rm_lines]
...
# print dataframes for x-checks
# out_f = home_dir + 'phasing_analysis/indel_df_troubleshooting/indels_dad.txt'
# indel_df_dad.to_csv(out_f, sep='\t')
# out_f = home_dir + 'phasing_analysis/indel_df_troubleshooting/indels_mom.txt'
# indel_df_mom.to_csv(out_f, sep='\t')
...
for i_class in indel_list:
    ...
    total_list_dad = indel_df_dad[i_class]  # .tolist()
    total_list_mom = indel_df_mom[i_class]
    # print(len(age_list_dad))
    # print(len(age_list_mom))
