...
age_f_loc = home_dir + 'data/parental_age_at_conception.txt'
parental_age = pd.read_csv(age_f_loc, sep='\t', engine='python')
parental_age.set_index(['ID'], inplace=True)
tot_loc = home_dir + 'indel_analysis/dnv_parent_totals_by_ID.txt'
parental_totals = pd.read_csv(tot_loc, sep='\t', engine='python')
...
paternal_data['All'] = (
    paternal_data['HR'] + paternal_data['CCC'] + paternal_data['non-CCC'])
maternal_data['All'] = (
    maternal_data['HR'] + maternal_data['CCC'] + maternal_data['non-CCC'])

indel_list.append('All')
length = len(indel_list)

pacbio_p_dict = {'Dad': {}, 'Mom': {}}
for i in range(length):
    print(indel_list[i])
    # paternal_mut_df = paternal_data[paternal_data.Indel_Class == indel_list[i]]
    # maternal_mut_df = maternal_data[maternal_data.Indel_Class == indel_list[i]]
    ...
    pacbio_p_dict['Dad'][indel_list[i]] = pearsonr(
        age_list_dad, total_list_dad)[1]
    print('Mom totals: {}, correlation:'.format(sum(total_list_mom)))
    pearsonr(age_list_mom, total_list_mom)
    pacbio_p_dict['Mom'][indel_list[i]] = pearsonr(
        age_list_mom, total_list_mom)[1]
