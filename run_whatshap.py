...
def whatshap(ID):
    ...
    for i in range(1, 23):
        ...
        print("========WhatshapData object created for patient family " + ID + " on chromosome " + str(i));
        command = whatshap_object.cmd();
        print("--------Running whatshap for patient family " + ID + " on chromosome " + str(i));
        subprocess.call(command, shell=True);
        print("********Succesfully ran whatshap for patient family " + ID + " on chromosome " + str(i));

if __name__ == '__main__':
    pool = mp.Pool(processes=6);
    pool.map(whatshap, patientID[1]);
